...
# Week 1 실행 팩(바로 복붙 실행)

> 아래 쿼리는 **소스 테이블**: `ns-extr-data.analytics_310486481.events_fresh_20251106` 기준입니다. 결과를 임시 테이블로 남겨 2주차에서 재사용합니다. 필요 시 `ns_exploration` 대신 사내 개발용 데이터셋명으로 교체하세요.

## 1) Mall 소스값 수집
**1-1. 최근 60일 소스 분포**
```sql
CREATE SCHEMA IF NOT EXISTS `ns_exploration`;
CREATE OR REPLACE TABLE `ns_exploration.mall_sources_60d` AS
SELECT LOWER(traffic_source.source) AS source,
       COUNT(*) AS events,
       COUNT(DISTINCT COALESCE(CAST(user_id AS STRING), user_pseudo_id)) AS users,
       COUNT(DISTINCT CONCAT(COALESCE(CAST(user_id AS STRING), user_pseudo_id), ':', CAST(p.int_value AS STRING))) AS sessions
FROM `ns-extr-data.analytics_310486481.events_fresh_20251106` t
LEFT JOIN UNNEST(t.event_params) p ON p.key='ga_session_id'
WHERE PARSE_DATE('%Y%m%d', t.event_date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
GROUP BY 1
...
**1-2. 소스×미디엄 상위 조합**
```sql
CREATE OR REPLACE TABLE `ns_exploration.mall_source_medium_60d` AS
SELECT LOWER(traffic_source.source) AS source,
       LOWER(traffic_source.medium) AS medium,
       COUNT(*) AS events
FROM `ns-extr-data.analytics_310486481.events_fresh_20251106`
WHERE PARSE_DATE('%Y%m%d', event_date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
GROUP BY 1,2
ORDER BY events DESC
LIMIT 1000;
```
...
**1-3. 캠페인명 후보**
```sql
CREATE OR REPLACE TABLE `ns_exploration.mall_campaigns_60d` AS
SELECT LOWER(traffic_source.name) AS campaign,
       COUNT(*) AS events
FROM `ns-extr-data.analytics_310486481.events_fresh_20251106`
WHERE PARSE_DATE('%Y%m%d', event_date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
GROUP BY 1
...
## 2) 핵심 파라미터 키 검증
**2-1. event_params 키 인덱스**
```sql
CREATE OR REPLACE TABLE `ns_exploration.ga4_param_census_60d` AS
SELECT ep.key,
       COUNT(*) AS occurrences,
       COUNTIF(ep.value.string_value IS NOT NULL) AS str_cnt,
       COUNTIF(ep.value.int_value    IS NOT NULL) AS int_cnt,
       COUNTIF(ep.value.double_value IS NOT NULL) AS dbl_cnt
FROM `ns-extr-data.analytics_310486481.events_fresh_20251106`, UNNEST(event_params) ep
WHERE PARSE_DATE('%Y%m%d', event_date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
GROUP BY ep.key
...
**2-2. 필수 키 존재율**
```sql
CREATE OR REPLACE TABLE `ns_exploration.ga4_param_required_60d` AS
WITH base AS (
  SELECT event_name, event_date, event_params FROM `ns-extr-data.analytics_310486481.events_fresh_20251106`
  WHERE PARSE_DATE('%Y%m%d', event_date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
)
SELECT 'ga_session_id' AS key,
  SAFE_DIVIDE(SUM(CASE WHEN (SELECT COUNT(1) FROM UNNEST(event_params) p WHERE p.key='ga_session_id' AND p.value.int_value IS NOT NULL) > 0 THEN 1 ELSE 0 END), COUNT(*)) AS presence_rate
FROM base
UNION ALL
SELECT 'engagement_time_msec', SAFE_DIVIDE(SUM(CASE WHEN (SELECT COUNT(1) FROM UNNEST(event_params) p WHERE p.key='engagement_time_msec') > 0 THEN 1 ELSE 0 END), COUNT(*))
FROM base
UNION ALL
SELECT 'page_location', SAFE_DIVIDE(SUM(CASE WHEN (SELECT COUNT(1) FROM UNNEST(event_params) p WHERE p.key='page_location' AND p.value.string_value IS NOT NULL) > 0 THEN 1 ELSE 0 END), COUNT(*))
FROM base
UNION ALL
SELECT 'transaction_id', SAFE_DIVIDE(SUM(CASE WHEN (SELECT COUNT(1) FROM UNNEST(event_params) p WHERE p.key='transaction_id') > 0 THEN 1 ELSE 0 END), COUNT(*))
FROM base
UNION ALL
SELECT 'value', SAFE_DIVIDE(SUM(CASE WHEN (SELECT COUNT(1) FROM UNNEST(event_params) p WHERE p.key='value') > 0 THEN 1 ELSE 0 END), COUNT(*))
FROM base;
```

## 3) 시멘틱 뷰 v0 생성 (개발 데이터셋)
> 필요 시 데이터셋명을 교체하세요. 파티션 필터를 위해 `event_date` 기반으로 범위를 받는 매개변수형 템플릿 예시도 함께 제공합니다.
...
**3-1. semantic_user**
```sql
CREATE OR REPLACE VIEW `ns_exploration.semantic_user` AS
SELECT
  COALESCE(CAST(user_id AS STRING), user_pseudo_id) AS user_key,
  ANY_VALUE(traffic_source.source)   AS first_source,
  ANY_VALUE(traffic_source.medium)   AS first_medium,
...
**3-2. semantic_session**
```sql
CREATE OR REPLACE VIEW `ns_exploration.semantic_session` AS
WITH e AS (
  SELECT
    COALESCE(CAST(user_id AS STRING), user_pseudo_id) AS user_key,
    (SELECT CAST(p.int_value AS STRING) FROM UNNEST(event_params) p WHERE p.key='ga_session_id') AS ga_session_id,
...
**3-3. semantic_event**
```sql
CREATE OR REPLACE VIEW `ns_exploration.semantic_event` AS
SELECT
  TIMESTAMP_MICROS(event_timestamp) AS event_ts,
...
```sql
-- 기본 행 수 점검
SELECT COUNT(*) AS events FROM `ns_exploration.semantic_event`;
SELECT COUNT(*) AS sessions FROM `ns_exploration.semantic_session`;
SELECT COUNT(*) AS users FROM `ns_exploration.semantic_user`;

-- 키 무결성
SELECT COUNT(*) AS null_sessions FROM `ns_exploration.semantic_session` WHERE session_key IS NULL;
SELECT COUNT(*) AS null_user_in_session FROM `ns_exploration.semantic_session` WHERE user_key IS NULL;

-- 퍼널 대시 스케치
SELECT
  SAFE_DIVIDE(SUM(CASE WHEN event_name='add_to_cart' THEN 1 ELSE 0 END), SUM(CASE WHEN event_name='view_item' THEN 1 ELSE 0 END)) AS atc_rate,
  SAFE_DIVIDE(SUM(CASE WHEN event_name='purchase' THEN 1 ELSE 0 END), SUM(CASE WHEN event_name='add_to_cart' THEN 1 ELSE 0 END)) AS purchase_after_atc
FROM `ns_exploration.semantic_event`;
```

## 5) 용어사전 반자동 업데이트 템플릿
**5-1. Mall 소스 후보를 YAML로 출력(쿼리 결과를 복사해 값만 채우면 됨)**
```yaml
mall_sources:  # update from ns_exploration.mall_sources_60d
  - mall
  - nsmall_mall
  - ns-mall
  # + 여기에 상위 20개 실제 source 값 추가
aliases:
  'Mall 유입': ['mall','nsmall_mall','ns-mall','mall.nsmall','몰유입', '<<신규소스1>>', '<<신규소스2>>']
```

**5-2. 파라미터 키 사전(존재율 주석 포함)**
```yaml
params:
  ga_session_id: INT64   # presence: <from ga4_param_required_60d>
  engagement_time_msec: INT64  # presence: ...
  page_location: STRING  # presence: ...
  transaction_id: STRING # presence: ...
  value: FLOAT64         # presence: ...
```

## 6) 체크리스트 (Week 1 완료 기준)
- [ ] `mall_sources_60d`, `mall_source_medium_60d`, `mall_campaigns_60d` 테이블 생성
- [ ] `ga4_param_census_60d`, `ga4_param_required_60d` 테이블 생성
- [ ] `semantic_user|session|event` 뷰 생성 및 QA 쿼리 통과
- [ ] 용어사전 YAML에 실제 Mall 소스 상위 20개 반영
- [ ] 지표 정의서 초안(CVR/AOV/Revenue/ATC Rate 등) 확정 주석 달기



---

# TODO 미작성 항목 채워넣기 가이드
> 각 항목을 **입력 → 작업 → 산출물 → 합격기준(AC)**로 정리했습니다. 실제 운영값/조직 구조에 맞게 소폭 조정만 하면 됩니다.

## A. 지표 정의서(정의·필터·오차 규칙)
**입력**: Week1 파라미터 존재율, 이벤트 명세, 비즈니스 정책(취소/환불 처리)
**작업**:
1) KPI 목록 확정(CVR, AOV, Revenue, ATC Rate, Engaged Sessions 등)
2) 각 지표에 대하여: 집계식, 기간 기준(세션/날짜), 분모 분자, 필수 필터(취소/테스트 트래픽 제외), 측정 단위
3) 예외/에지케이스 규칙: 0 나누기 SAFE_DIVIDE, 다중 통화, 중복 트랜잭션 처리
**산출물**: `metrics_definitions.yaml`
```yaml
- name: conversion_rate
  title: 구매전환율(CVR)
  numerator: COUNT_IF(event_name='purchase')
  denominator: COUNT_DISTINCT(session_key)
  filters: [source IN @mall_sources, event_date BETWEEN @start AND @end]
  unit: ratio
  notes: 세션기반; 중복 transaction_id는 1건으로 de-dup
```
**AC**: 골든세트 15문항에서 계산 일치율 ≥ 95%, 리뷰어 2인 승인

## B. 시멘틱 레이어 정교화(관계·브리지·스냅샷)
**입력**: 자주 쓰는 조인 경로, 경로 분석 요구(2~3스텝), 다대다 위험 지점
**작업**:
- 세션↔이벤트 1:N, 유저↔세션 1:N 재확인, 다대다 시 브리지 테이블 설계(예: item-level 퍼널)
- 시간여행 규칙(사용자 속성 변경 이력) 필요 시 스냅샷 뷰 추가
**산출물**: `semantic.yml` 업데이트(relationship, grain 주석 강화)
**AC**: 퍼널·경로 질문 10문항에서 중복집계 이슈 0건

## C. 가드레일 정책(실행 전 안전장치)
**입력**: 비용 한도, 파티션 키, 민감 필드 목록
**작업**:
- DRY RUN 필수 체크, `maximum_bytes_billed` 기본값(예: 50GB) 설정
- 파티션 필터 미존재시 차단 룰
- 금칙어/민감항목 마스킹 규칙
**산출물**: `guardrails.json` + 에이전트 훅 코드
```json
{
  "deny_if": ["NO_PARTITION_PREDICATE", "SELECT_STAR_LARGE"],
  "require": ["DRY_RUN"],
  "defaults": {"maximum_bytes_billed": 50000000000}
}
```
**AC**: 비용 초과/무파티션 쿼리 100% 차단 로그 확인

## D. 프롬프트 템플릿·플래너 규칙
**입력**: 시멘틱 YAML, 용어사전, KPI 정의
**작업**:
- NL 파서: 기간·엔터티·지표 슬롯 채우기 규칙(한글 상대기간 포함)
- 플래너: 조인 경로/그룹바이/필터/윈도우 선택 규칙 테이블화
**산출물**: `prompt.sqlgen.txt`, `planner_rules.md`
**AC**: 골든세트 15문항 중 12문항 이상 **수정 없이** 유효 SQL 생성

## E. 골든세트 생성(정답 SQL·결과)
**입력**: 질문 40문항, 시멘틱 뷰, KPI 정의
**작업**:
1) 우선순위 15문항 선택(퍼널/경로/세션/매출)
2) 각 문항에 대해: 자연어→시멘틱 플랜→정답 SQL → 결과 스냅샷 테이블 저장
**산출물**: `goldenset/001_*.sql`, `goldenset/answers/*.parquet`
**AC**: 재실행 시 결과 해시 일치, 리뷰어 승인

## F. 성능·캐시(머티뷰/BI Engine)
**입력**: 자주 조회하는 쿼리 패턴, 비용 상위 테이블
**작업**:
- 일자×소스×이벤트 요약 머티뷰
- 퍼널 집계 테이블(일별)
- BI Engine 예약 용량(필요 시)
**산출물**: `materialized_views.sql`
```sql
CREATE MATERIALIZED VIEW IF NOT EXISTS ns_exploration.mv_events_daily AS
...
GROUP BY 1,2,3;
```
**AC**: 상위 대시 쿼리 평균 지연 50%↓, 스캔바이트 70%↓

## G. 보안·거버넌스(IAM/RLS/마스킹)
**입력**: 접근 그룹, 지역 권한, PII 필드
**작업**:
- Authorized View로 세그먼트 공개, user_id 해시 마스킹
- RLS 정책 함수: Mall 유입만 허용(옵션)
**산출물**: `security_policies.sql`
**AC**: 승인되지 않은 사용자 조회 불가(테스트 케이스 통과)

## H. 로그·관측(정확도/비용/지연)
**입력**: 에이전트 컨텍스트, BQ Job 메타
**작업**:
- Job labels: `user`, `intent`, `metric`, `mall_flag`
- 쿼리 결과와 해석 요약을 로그/테이블에 저장
**산출물**: `observability.ddl`, 대시보드 스케치
**AC**: 모든 호출에서 라벨 수집율 100%

## I. UX 세부(수정가능 제안/근거표시)
**입력**: 사용자 피드백, 실패 유형
**작업**:
- "해석 요약" 카드: 기간/채널 토글, 시멘틱 플랜 시각화
- 실패 시 대체 제안: 스키마 후보/기간 후보 제시
**산출물**: UX 사양서 `ux_spec.md` + 샘플 스크린샷
**AC**: 파일럿 사용자 5인 테스트 만족도 기준 충족(내부 스코어카드)

## J. 운영 규칙(사전 업데이트·회귀 테스트)
**입력**: 최신 소스 테이블, 실패 로그
**작업**:
- 매주 Mall 소스 자동 스캔 → 신규값 제안 → 승인 시 용어사전 병합
- 골든세트 회귀: 시멘틱/사전 변경 시 자동 실행
**산출물**: `cron_jobs.md` + 스크립트(Cloud Scheduler/Functions)
**AC**: 배포 후 정확도 하락 시 자동 알람
...
```json
{
  "intent": "funnel_summary",
  "entities": ["event","session"],
  "filters": [
    {"field":"source","op":"in","value":"@mall_sources"},
    {"field":"event_date","op":"between","value":["@start","@end"]}
  ],
  "steps": [
    {"event":"view_item"}, {"event":"add_to_cart"}, {"event":"purchase"}
  ],
  "group_by": ["d","device_category"]
}
```
**2) 실패 분석 코드북**
```yaml
...
- type: LOGIC_ERROR
  symptom: 퍼널 분모 불일치
  fix: grain 교정 or distinct 경로 적용
- type: COST_GUARD
  symptom: 무파티션/대용량 쿼리 차단
  fix: 기간 파라미터 주입
```
...
# 책임 배분(RACI) 예시
- **Responsible**: 데이터 애널리스트(지표 정의·골든세트), 데이터 엔지니어(뷰/머티뷰·보안), 앱 엔지니어(에이전트·UX)
- **Accountable**: 데이터 리드
- **Consulted**: 마케팅/CRM 오너, 웹분석 오너(GA4)
- **Informed**: 서비스 기획, CS

---

# 체크리스트 요약(빈칸 메우기용)
- [ ] KPI 정의서 완성(분모/분자/필터/단위)
- [ ] 시멘틱 관계 주석(1:N, 브리지) 명시
- [ ] 가드레일 JSON 배포 및 차단 로그 확인
- [ ] 프롬프트/플래너 규칙 버전관리 시작
- [ ] 골든세트 15문항 결과 스냅샷 저장
- [ ] 머티뷰/요약테이블로 비용·지연 절감 검증
- [ ] IAM/RLS/마스킹 테스트 통과
- [ ] 관측 대시보드 라벨 수집률 100%
- [ ] UX 스펙과 실패 대체 제안 플로우 합의
- [ ] 주간 사전 스캔 잡 동작 확인
